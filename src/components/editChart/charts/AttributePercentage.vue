...
<template>
  <div class="chart-container flexOne" v-loading="loading">
    <div class="chart-header">
      <div class="chart-title">舆情属性占比</div>
...
export default {
  ...
  methods: {
     ...
     * @param 父级参数
     */
    getInitData(publicData) {
      const updateTable = [
        { name: "非敏感", value: publicData.nonsensitivity },
        { name: "敏感", value: publicData.sensitivity },
      ]
      this.echoParentsData(updateTable, this.editFormData)
    },
    // 初始化渲染charts
    initChart() {
      if (!this.myChart) {
