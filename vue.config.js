...
// 扩展自定义debug模式
if (envMode && typeof envMode === 'string') {
  envMode = envMode.trim();
}

require('./config/index.evn')();
...
module.exports = {
  ...
  publicPath: process.env.NODE_ENV === 'production' ? '/' : '/', // 发布时配置的静态地址前缀
  ...
  //  devServer: {},
  devServer: {
    // port: configDev.devProxy.port, // 端口号
    // host: configDev.devProxy.host,
    https: false, // https:{type:Boolean}
    open: false, // 配置自动启动浏览器
    // proxy: 'http://localhost:4000' // 配置跨域处理,只有一个代理
    proxy: developHelper.getProxy(),  // 配置多个代理
    before (app) {
      if (envMode === 'mock') {
